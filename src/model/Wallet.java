...
public class Wallet {
        ...
        public List<Pocket> putObjectIn(int pocketNumber, WalletObject object) {
        ...
        public List<Pocket> putObjectOut(int objectId) {
            WalletObject object = getObjects().stream().filter(obj -> obj.getId() == objectId)
                    .toList().get(0);
        ...
        public List<WalletObject> getObjects() {
            List<WalletObject> objects = new ArrayList<>();
            pockets.stream().map(Pocket::getObjects)
                    .forEach(objects::addAll);
            return objects;
        }

        public List<WalletObject> getObjectIn(int pocketNumber) {
        ...
        public Double getBalance() {
            ...
            pockets.stream().map(Pocket::getObjects)
                    .forEach(objectList -> {
                        for (WalletObject obj : objectList) {
